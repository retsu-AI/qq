...
/// Schema 40 adds the nullable Jev mode to a pre-40 store, leaves existing
/// sessions on the configured capabilities, and refuses a column that already
/// exists with the wrong shape.
#[test]
fn version_thirty_nine_gains_the_jev_mode_and_rejects_a_bad_shape() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("sessions.sqlite3");
    let (connection, store_id) = open_database(&path).unwrap();
    connection
        .execute_batch(
            "ALTER TABLE sessions DROP COLUMN jev_mode;
             INSERT INTO workspaces(id, path) VALUES ('ws', '/tmp/ws');
             INSERT INTO sessions(
                 id, workspace_id, title, status, approval_mode, created_at_ms, updated_at_ms,
                 reasoning_effort, approval_delegate
             ) VALUES ('s1', 'ws', 'old', 'idle', 'ask', 0, 0, 'default', 'off'),
                      ('s2', 'ws', 'other', 'idle', 'ask', 0, 0, 'max', 'on');
             UPDATE metadata SET value = '39' WHERE key = 'schema_version';",
        )
        .unwrap();
    drop(connection);

    let (connection, reopened_id) = open_database(&path).unwrap();
    assert_eq!(reopened_id, store_id);
    assert!(has_column(&connection, "sessions", "jev_mode").unwrap());
    assert_eq!(
        connection
            .query_row(
                "SELECT value FROM metadata WHERE key = 'schema_version'",
    ...
    let stored: Option<String> = connection
        .query_row("SELECT jev_mode FROM sessions WHERE id = 's1'", [], |row| {
            row.get(0)
        })
        .unwrap();
    assert_eq!(
        stored, None,
        "pre-40 sessions keep the configured Jev roles"
    );
    let preserved: Vec<(String, String, String)> = connection
        .prepare("SELECT id, reasoning_effort, approval_delegate FROM sessions ORDER BY id")
        .unwrap()
    ...
    assert_eq!(
        preserved,
        vec![
            ("s1".to_owned(), "default".to_owned(), "off".to_owned()),
            ("s2".to_owned(), "max".to_owned(), "on".to_owned()),
        ]
    );
    connection
        .execute_batch(
            "ALTER TABLE sessions DROP COLUMN jev_mode;
             ALTER TABLE sessions ADD COLUMN jev_mode INTEGER NOT NULL DEFAULT 0;",
        )
        .unwrap();
    drop(connection);
...
#[tokio::test]
async fn version_forty_migration_preserves_legacy_runs_without_verification() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("sessions.sqlite3");
    ...
    store.close().await.unwrap();
    let connection = Connection::open(&path).unwrap();
    connection.execute_batch("ALTER TABLE runs DROP COLUMN verification_json; ALTER TABLE tool_calls DROP COLUMN verification_reviewed; UPDATE metadata SET value = '40' WHERE key = 'schema_version';").unwrap();
    drop(connection);
    let (connection, _) = open_database(&path).unwrap();
    let legacy = load_run(&connection, id).unwrap();
    assert_eq!(legacy.outcome, Some(RunOutcome::Completed));
    ...
    connection.execute_batch("ALTER TABLE runs DROP COLUMN verification_json; ALTER TABLE runs ADD COLUMN verification_json INTEGER NOT NULL DEFAULT 0;").unwrap();
    drop(connection);
    assert!(matches!(
        open_database(&path),
        Err(SessionRuntimeError::CONSTRAINT)
