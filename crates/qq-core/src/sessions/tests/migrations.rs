...
/// Schema 37 adds the nullable Jev mode to a pre-37 store, leaves existing
/// sessions on the configured capabilities, and refuses a column that already
/// exists with the wrong shape.
#[test]
fn version_thirty_six_gains_the_jev_mode_and_rejects_a_bad_shape() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("sessions.sqlite3");
    let (connection, _) = open_database(&path).unwrap();
    connection
    ...
    assert_eq!(
        stored, None,
        "pre-37 sessions keep the configured Jev roles"
    );
    connection
        .execute_batch(
            "ALTER TABLE sessions DROP COLUMN jev_mode;
