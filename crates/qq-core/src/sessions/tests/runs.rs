...
#[tokio::test]
async fn strict_cancellation_and_deadline_retain_unavailable_and_unknown_spend() {
    struct Loader;
    impl RuntimeLoader for Loader {
        fn load(&self, request: RuntimeLoadRequest) -> RuntimeLoadFuture {
            struct Reviewer;
    ...
    for cancel in [true, false] {
        ...
        let queued = runtime
            .command(
                ...
                SessionCommand::SubmitPrompt {
                    session_id,
                    input: vec![InputPart::text("inspect")],
                    limits: RunLimits {
                        max_duration_ms: Some(if cancel { 2000 } else { 250 }),
                        ..RunLimits::default()
                    },
                    correlation: Correlation::default(),
                    output: None,
        ...
        let mut observed = collect_until(&mut events, |e| {
            matches!(
                e,
        ...
        if cancel {
            assert_eq!(*terminal.0, RunOutcome::Cancelled);
        } else {
            assert!(
                matches!(terminal.0, RunOutcome::BudgetExhausted { exhaustion } if exhaustion.limit == qq_protocol::BudgetLimitKind::Duration)
            );
        }
        let reviewed = observed.iter().filter(|e| matches!(&e.event, SessionEvent::CheckpointReviewed { outcome: qq_protocol::CheckpointOutcome::Unavailable, spend: Some(spend), .. } if spend.usage.is_none() && spend.estimated_cost_usd_nanos.is_none())).count();
        assert_eq!(reviewed, 1);
        let cold = runtime
            .snapshot(SnapshotRequest::new(workspace_id, Some(session_id), 1, 8))
            .await
            .unwrap();
        assert_eq!(
            cold.focused
        ...
        let mut replay = runtime
            ...
            .unwrap();
        let replayed = collect_until(&mut replay, finished_for(run_id)).await;
        assert_eq!(
