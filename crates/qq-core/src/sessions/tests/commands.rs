...
#[tokio::test]
async fn set_jev_mode_persists_publishes_and_is_inherited_by_children() {
    let mut harness = session_management_harness().await;
    let receipt = harness
        .runtime
        .command(
    ...
    // The receipt names the committed `session_updated` cursor: the row and
    // the event are one transaction, and the stream publishes after it.
    let updated = harness.events.next().await.unwrap().unwrap();
    ...
    assert!(matches!(
        ...
        SessionEvent::SessionUpdated { session }
            if session.id == harness.session_id
                && session.jev_mode == Some(qq_protocol::JevMode::Ultrajev)
    ));
    let connection = Connection::open(harness.directory.path().join("sessions.sqlite3")).unwrap();
    let stored: Option<String> = connection
        .query_row(
            "SELECT jev_mode FROM sessions WHERE id = ?1",
            [harness.session_id.to_string()],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(stored.as_deref(), Some("ultrajev"));
    drop(connection);

    // Snapshots (and so reconnects) rebuild the same value.
    let snapshot = harness
        .runtime
        .snapshot(SnapshotRequest {
            workspace_id: harness.workspace_id,
        ...
        .unwrap();
    assert_eq!(
        snapshot.focused.unwrap().summary.jev_mode,
        Some(qq_protocol::JevMode::Ultrajev)
    );

    // A child created under the session starts with the parent's mode.
    ...
    let created = harness.events.next().await.unwrap().unwrap();
    assert!(matches!(
        &created.event,
        SessionEvent::SessionCreated { session }
            if session.id == child_id && session.jev_mode == Some(qq_protocol::JevMode::Ultrajev)
    ));

    // Clearing restores the configured capabilities and is published too.
    harness
        .runtime
        .command(
            CommandId::generate().unwrap(),
    ...
    assert!(matches!(
        &cleared.event,
        SessionEvent::SessionUpdated { session }
            if session.id == harness.session_id && session.jev_mode.is_none()
    ));
    ...
    let missing = harness
        ...
        .command(
            ...
            SessionCommand::SetJevMode {
                ...
                mode: Some(qq_protocol::JevMode::Low),
            },
        )
        .await
        .unwrap_err();
    assert_eq!(missing, SessionRuntimeError::SessionNotFound);
}

#[tokio::test]
async fn jev_mode_reaches_the_loader_at_the_next_claim_not_the_active_run() {
    struct JevModeRecordingLoader {
        modes: Arc<StdMutex<Vec<Option<qq_protocol::JevMode>>>>,
        requests: Arc<StdMutex<Vec<ModelRequest>>>,
    }

    impl RuntimeLoader for JevModeRecordingLoader {
        fn load(&self, request: RuntimeLoadRequest) -> RuntimeLoadFuture {
            self.modes.lock().unwrap().push(request.jev_mode);
    ...
    let runtime = SessionRuntime::open(
        SessionRuntimeOptions::new(directory.path().join("sessions.sqlite3")),
        Arc::new(JevModeRecordingLoader {
    ...
    // The first run is claimed with no mode and parks at its approval.
    let run_id = submit_prompt_to(&runtime, session_id, "first").await;
    let (_, tool_call) = collect_until_approval_requested(&mut events).await;
    assert_eq!(modes.lock().unwrap().as_slice(), &[None]);

    // Switching mid-run is durable and published, but the executing run
    // keeps the plan it compiled: no reload happens.
    runtime
        .command(
            CommandId::generate().unwrap(),
            SessionCommand::SetJevMode {
                session_id,
                mode: Some(qq_protocol::JevMode::Max),
            },
        )
        .await
        .unwrap();
    let updated = events.next().await.unwrap().unwrap();
    assert!(matches!(
        &updated.event,
        SessionEvent::SessionUpdated { session }
            if session.jev_mode == Some(qq_protocol::JevMode::Max)
    ));
    respond_approval(&runtime, run_id, tool_call.id, ApprovalDecision::Deny)
        .await
        .unwrap();
    collect_through_finished(&mut events).await;
    assert_eq!(modes.lock().unwrap().as_slice(), &[None]);

    // The next claim reads the row and hands the mode to the loader.
    let run_id = submit_prompt_to(&runtime, session_id, "second").await;
    let (_, tool_call) = collect_until_approval_requested(&mut events).await;
    assert_eq!(
        modes.lock().unwrap().as_slice(),
        &[None, Some(qq_protocol::JevMode::Max)]
    );
    respond_approval(&runtime, run_id, tool_call.id, ApprovalDecision::Deny)
        .await
        .unwrap();
