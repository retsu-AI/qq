...
**Date:** 2026-09-24
**Deciders:** integration plan § 2 item 9 (two model switchers);
manager-directed integration on 2026-09-25 preserves the existing ladder
...
thinking-budget, or mode parameter, and Jev returns typed decisions, not
...
2. **`JevMode` is a session-level ladder over the existing Jev roles.**
   ...
   The ladder is monotone: successive steps enable additional review or delegation defaults. The
...
3. **A session override, like `approval_delegate`.** New command
   ...
   `sessions.jev_mode` column (schema 40). The command writes the row and the
...
- Protocol 30 → 31 (new command, outcome, summary field, reserved slash
  command); store schema 39 → 40. Older clients reject the new command and
...
- An operator turns Jev up or down for one session from any surface without
  a config write or restart: the `/jev` picker in the TUI, `POST
  /v1/sessions/jev-mode` from a web or mobile client, and every attached
  client sees `session_updated.jev_mode` change.
- This integration preserves the existing proposed ladder. It does not
  establish approval of a new strict completion policy. The mapping remains
  one composition-root table; workspace consent and session delegate
  overrides can make adjacent rows equivalent in a particular workspace.
- Jev request bounds, thresholds and identities are unchanged. Under RR3,
...
- **A `JevMode` that pins a `model` alias** (`jev-latest` / `jev-preview`) was
  ...
  versioned id deliberately so the request contract stays versioned (ADR-0034,
