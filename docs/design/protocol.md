...
A store keeps every command receipt as its idempotency record and bounds
their number. Commands that create work are refused (`503`, "durable command
limit reached") at the bound; `cancel_run`, `respond_tool_approval`,
`delete_session`, `prune_sessions`, and `rollback_compaction` are admitted
past it up to a reserved headroom, so a client can always stop and clean up
a full store. Replay of an earlier `command_id` returns its receipt at any
fill level.
