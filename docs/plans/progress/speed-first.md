...
| H22.2 | Structural bundle: `COMMAND_ROUTES`, `Box<SessionSummary>`, config/auth load, TUI; `Notify` cancellation stacked | In review | `refactor/h22-2-structural-bundle` → `perf/h22-2-notify-cancellation` | 29 items across 8 crates, one commit per crate; `StaticHttpAuth`, headless writer, config parse-once, reviewer-via-PlanCache deferred with reasons in the plan |
