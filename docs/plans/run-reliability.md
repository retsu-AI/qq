...
| RR6 | Mid-run compaction at a tool boundary; provider `ContextExceeded` marks occupancy full and compacts next turn; exhausted fold admits with summary-only history | R02 | `crates/qq-core/src/sessions/{execution,context,compaction,transcript}.rs`, `lib.rs` plan step | Three compactions inside one run; 413 on turn 7 recovers; `Exhausted` session accepts next prompt; independent review required (touches `sessions/`) |
...
### Mid-run compaction (RR6)

C2 (`3446c54`) proved the store can stub stale tool results at a turn
boundary inside a run. RR6 extends that: when `context::plan` returns
`Compact` on turn *n*, the run loop commits the current turn, runs one bounded
summarizer step over the run's *own* turns 1..*n* plus prior history
(`load_summarizer_input` already bounds this), commits a compaction marker at
ordinal *n*, and continues with `[summary] + retained suffix`. `BetweenRunsOnly`
becomes `Eligible` for turns > 1 with a per-run step cap of 4. A provider
`ContextExceeded` after an estimated `Send` sets `context_occupancy` to the
window and takes the same path on the next turn. At admission,
`Exhausted(Attempted)` starts the run with the last marker as sole history and
a runtime notice rather than rejecting.
...
continuation. Once RR6 ships, evaluate deleting the checkpoint: mid-run
compaction is the durable checkpoint, and RR12's loop result covers the
runaway case.
...
3. `docs/design/architecture.md` § run loop and `tools.md` § Loop Bounds
   amended; ADR for two-phase retry ownership accepted; `headless-contract.md`
   documents `Paused`.
